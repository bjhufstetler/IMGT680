...
#### Commentary: Note that birch is no longer supported on CRAN and must be installed through an archived version. The approval and interest were removed from the classification training data because the interest is perfectly correlated with the requested amount.
...
### PLOT THE RESULTS
#### Commentary: Since only one value of k is selected (2), we cannot make a comparison to different models and BIRCH is unable to suggest a k. The clusters shown below 
```{r}
  par(mfrow=c(2,2))
...
#### Commentary: Min-max normalization used (from chapter 2)
```{r echo=T, results='hide'}
